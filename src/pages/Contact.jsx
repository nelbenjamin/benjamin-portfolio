...
import { Mail, Phone, MapPin, ExternalLink, Send, User, MessageSquare } from 'lucide-react';
...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    ...
    window.location.href = `mailto:benjaminintja@gmail.com?subject=${subject}&body=${body}`;
    setSent(true);
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 32 }} className="contact-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {CONTACT_INFO.map((info, i) => {
            const content = (
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className="card-hover" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 22, display: 'flex', alignItems: 'center', gap: 16, cursor: info.href ? 'pointer' : 'default' }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(41,121,255,0.12)', border: '1px solid rgba(41,121,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
            style={{ background: 'linear-gradient(135deg, #0d2444, #0a1525)', border: '1px solid var(--border-bright)', borderRadius: 16, padding: 22, marginTop: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <div className="pulse-ring" style={{ width: 8, height: 8, borderRadius: '50%', background: '#00c896' }} />
              <span style={{ fontSize: 13, fontWeight: 700, color: '#00c896' }}>Currently Available</span>
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
          ...
          {sent ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: 'rgba(0,200,150,0.12)', border: '1px solid rgba(0,200,150,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                <Send size={24} color="#00c896" />
              </div>
              <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>Opening your email client...</h3>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>If nothing opened, email me directly at benjaminintja@gmail.com</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <motion.button type="submit" whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} className="btn-primary"
